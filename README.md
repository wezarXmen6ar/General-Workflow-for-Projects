...
| Draft | A small slice moves from the backlog into the draft; only one draft is open at a time. Before a push, every entry needs its full chain up to an objective. |
...
- **Drafts:** `drafts/backlog/` for every idea not in the open draft; `drafts/vX/` for each push (pushed drafts stay as frozen records).
...
python tools/workflow.py new-draft v0.2 # start the next draft (when no draft is open)
