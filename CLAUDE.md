...
- plan/map.html shows everything pushed. The backlog and the draft each have a map beside them showing the whole pushed plan, plain, for context, with their own entries on top. Features built in the prototype or the product show a ✓. Once anything is pushed, items not pushed yet are marked NEW, amendments AMENDED, retirements CUT, and new links are highlighted; a new link between two items that already existed has its own colour. A written breakdown of the changes sits under the map, and a "Show only changes" button hides the plain items that are not on a changed item's chain.
- Every pushed item carries a small chip with the version it was first pushed in (e.g. v0.1). Its detail strip adds a history line: pushed in, amended in, built in (a saved prototype), accepted in (a saved review), released in.
- Each version keeps a map of every current item at that version, with that version's changes marked, including the items it retires, and the chips, history, and ✓ as of that version (versions/vX/mindmap.html). The first version has nothing to compare with, so nothing is marked. Saved maps never change.
- Which map to open: today's plan is plan/map.html; the next change is the draft's map; a past version is versions/vX/mindmap.html.
