...
The next push, kept small. New entries have no ID yet; they get one when the draft is pushed, and the draft then moves into its version folder (versions/v0.1/) and is frozen there. Refer to pushed items by ID (S-002) and to entries in this draft by their exact title in double quotes ("Title"). Put any explanation after " — ".
