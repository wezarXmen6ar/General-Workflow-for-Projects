...
def draft_graph(draft, main, built=frozenset(), full=False):
    """The draft's own entries over pushed context: with `full`, the whole plan (for the map); otherwise all
    objectives and problems plus only the chains this draft touches (for the written overview)."""
    highlight = bool(main.items)
    note = (f"The whole plan, with what {draft.path.name} adds or changes marked \u2014 generated by {TOOL}; "
            "edit the text, not this map") if full and main.items else \
        f"Generated from {draft.path.name} by {TOOL} \u2014 edit the text, not this map"
    g = Graph(draft.title, note, highlight)
    include = {it.id for it in main.active() if full or it.type in ("objective", "problem")}
    amended = {}
    for e in draft.entries:
...
def version_graph(version, main, prev, built=frozenset()):
    ...
    for it in main.ordered():
        ...
        if it.retired:
            ...
            mark = "amended"
        g.node(it.id, it.type, it.title, short_label(it.title, it.label), it.id, mark, it.seq, it.id in built)
...
def draft_outputs(draft, main, built=frozenset(), hist=None):
    """The regenerated draft text and map; `missing` lists generated blocks without markers."""
    g = draft_graph(draft, main, built)
    m = annotate(draft_graph(draft, main, built, full=True), history() if hist is None else hist, built, in_draft=True)
    text = draft.tf.text
    missing = []
    for name, content in (("tree", g.tree()), ("copied", copied_block(main))):
        ...
        if new is None:
            ...
            text = new
    return text, render_map(m), missing
...
def history(extra=None):
    """Item ID -> what happened to it: pushed in, amended in, built in (a saved prototype), accepted in (a saved
    review), released in (a release's Built line). `extra` = (version, Main) adds a version being recorded now."""
    rev = reversed_versions()
    steps = [(f.name, Main(f, Issues()), f) for f in version_folders() if f.name not in rev]
    if extra:
        steps.append((extra[0], extra[1], None))
    h, prev = {}, None
    for name, m, folder in steps:
        for iid, it in m.items.items():
            e = h.setdefault(iid, {"since": name, "amended": [], "built": "", "accepted": "", "released": ""})
            old = prev.items.get(iid) if prev else None
            if old and m.entry_text(it, drop_backlinks=True) != prev.entry_text(old, drop_backlinks=True):
                e["amended"].append(name)
        proto = folder / "prototype" if folder else None
        if proto and proto.exists():
            for f, text in text_files(proto):
                if f.suffix.lower() not in (".html", ".htm", ".js") or "trace" in f.relative_to(proto).parts:
                    continue
                for ln in text.split("\n"):
                    for chunk in TRACE_ATTR.findall(ln) + TRACE_JS.findall(ln):
                        for iid in ids_in(chunk):
                            if iid in h and not h[iid]["built"]:
                                h[iid]["built"] = f"the {name} prototype"
        for path in ((folder / "prototype" / "REVIEW.md", folder / "review.md") if folder else ()):
            vals = read_review(path)
            if review_filled(vals):
                for iid in ids_in(vals.get("accepted")):
                    if iid in h and not h[iid]["accepted"]:
                        h[iid]["accepted"] = f"the {name} review"
        prev = m
    cur = None
    for ln in log_lines():
        ...
        m = re.match(r"^-\s+Built:\s*(.*)$", ln)
        if m and cur and cur not in rev:
            for iid in ids_in(m.group(1)):
                if iid in h and not h[iid]["released"]:
                    h[iid]["released"] = cur
    return h


def annotate(g, h, built_now=frozenset(), in_draft=False):
    """Give every node its version chip and a one-line history for the map's detail strip."""
    for n in g.nodes.values():
        e = h.get(n["ref"]) if n["ref"] else None
        if not e:
            if not n["ref"]:
                n["hist"] = "Not pushed yet: new in this draft" if in_draft else ""
            continue
        n["since"] = e["since"]
        parts = [f"Pushed in {e['since']}"]
        if e["amended"]:
            parts.append("Amended in " + ", ".join(e["amended"]))
        if in_draft and n["mark"] == "amended":
            parts.append("Changed again by this draft")
        if in_draft and n["mark"] == "cut":
            parts.append("Retired by this draft")
        if e["built"]:
            parts.append(f"Built in {e['built']}")
        elif n["ref"] in built_now:
            parts.append("Built, not saved in a version yet")
        if e["accepted"]:
            parts.append(f"Accepted in {e['accepted']}")
        if e["released"]:
            parts.append(f"Released in {e['released']}")
        n["hist"] = " · ".join(parts)
    return g
...
def run_checks(ctx):
    ...
    built, hist = built_features(main), history()
    if not PLAN_MAP.exists() or norm_text(TextFile(PLAN_MAP).text) != render_map(annotate(plan_graph(main, built), hist, built)):
    ...
    for d in ctx.drafts:
        ...
        text, html, missing = draft_outputs(d, main, built, hist)
        for name in missing:
...
def build(ctx, writer):
    ...
    built, hist = built_features(ctx.main), history()
    writer.write(PLAN_MAP, render_map(annotate(plan_graph(ctx.main, built), hist, built)))
    for d in ctx.drafts:
        text, html, missing = draft_outputs(d, ctx.main, built, hist)
        for name in missing:
...
def cmd_record_push(args):
    ...
    text, html, _ = draft_outputs(d, before, built_features(before), history())
    ...
    writer.write(target / "mindmap.html", render_map(annotate(version_graph(v, ctx.main, prev, built_now),
                                                              history((v, ctx.main)), built_now)), allow_frozen=True)
    # 3. the log entry
    ...
    build(Context(), writer)  # the maps' version chips and history lines change with it
...
def cmd_record_release(args):
    ...
    shipped = built_features(ctx.main) & {iid for _, _, iid in product_ids()}
    log = with_log_line(log, v, "Built", ", ".join(it.id for it in ctx.main.ordered()
                                                    if it.id in shipped) or "none")
    if fixed:
